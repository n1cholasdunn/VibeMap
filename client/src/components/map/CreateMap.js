...
function CreateMap() {
	...
	const [destination, setDestination, clearDestination] = useContext(DestinationContext)
	...
	const sortedLocations = locations.sort((a, b) => {
		const distanceA = getDistance(
			{ latitude: destination.coords.start.lat, longitude: destination.coords.start.lng },
		...
		<>
			<div className="flex flex-row justify-center p-6 z-[-1]" >
				<div className="flex flex-col  justify-end w-4/5 h-auto aspect-w-1 aspect-h-1">
					...
					<form
						className="flex flex-row py-2 self"
				...
				<div className="pl-6 pr-6">
					...
					<div className="overflow-scroll overflow-y-scroll no-scrollbar h-[500px]">
						{sortedLocations.filter(filteredLocations).map(location => (
							<PlaceCard location={location} handleAddPoint={handleAddPoint}
								onClick={() => openPlaceInfo(location.lat, location.lng, location.name)} />))}
					</div>
				</div>
			</div >
