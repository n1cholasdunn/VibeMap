...
import { editUserTrip, postUserTrip } from "../../services/tripService";
...
function ViewMap({ trip }) {
    ...
    const center = useMemo(() => ({ lat: trip.coords.start.lat, lng: trip.coords.start.lng }), []);
        ...
        <div className="flex flex-row p-6 z-[-1]" >
            <div className="flex flex-col justify-end w-4/5 h-auto aspect-w-1 aspect-h-1">
                ...
                {!isLoaded ? (
                    ...
                    <GoogleMap mapContainerClassName="map-container" center={center} zoom={20}>
                        ...
                        <>
                            <Marker
                                position={{ lat: trip.coords.start.lat, lng: trip.coords.start.lng }}
                                icon={"http://maps.google.com/mapfiles/ms/icons/green-dot.png"}
                            />

                            {trip.coords.midpoint !== null && trip.coords.midpoint !== undefined && (
                                <Marker
                                    position={{ lat: trip.coords.midpoint.lat, lng: trip.coords.midpoint.lng }}
                                    icon={"http://maps.google.com/mapfiles/ms/icons/yellow-dot.png"}
                                />
                            )}
                            {trip.coords.end !== null && trip.coords.end !== undefined && (
                                <Marker
                                    position={{ lat: trip.coords.end.lat, lng: trip.coords.end.lng }}
            ...
            <div className="pl-6 pr-6">
                ...
                <div className="overflow-scroll h-[500px] w-[500px] overflow-y-scroll no-scrollbar">
                    ...
                    {trip.points.map(location => (
                        <div
                            className="cursor-pointer label border-solid border-gray-200 border rounded-md mt-3 pl-3 pr-5 ">

                            <div flex flex-col >
                                <div className="font-semibold text-lg">{location.name}</div>
                                <div className="font-normal text-sm">{location.address}</div>

                                <div className="font-medium text-md bg-gradient-to-r from-blue-600 to-indigo-400 inline-block text-transparent bg-clip-text">Vibe: </div>
                                <div className="flex flex-row">
                                    {location.categories.map((category) => (
                                        <p className="pr-2 text-sm bg-gradient-to-r from-orange-600 to-gray-400 inline-block text-transparent bg-clip-text ">{category}</p>
                                    ))}
                                </div>
                            </div>
                            <div>
